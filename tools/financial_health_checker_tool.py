...
class FinancialHealthCheckerTool(Tool):
    ...
    def cal_emergency_fund_ratio(self, current_assets: float, monthly_nondiscreationary_expenses: float):
        return current_assets / monthly_nondiscreationary_expenses

    def cal_net_worth(self, assets: float, liabilities: float):
        return assets - liabilities

    def cal_liquidity_ratio(self, current_assets: float, current_liabilities):
        return current_assets / current_liabilities

    def cal_saving_and_investing_ratio(self, savings: float, investments: float, monthly_income: float):
        return (savings + investments) / monthly_income

    def cal_financial_freedom_ratio(self, passive_income: float, expenses: float):
